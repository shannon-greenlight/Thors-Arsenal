...
extern String
csv_elem(String s, char delim, int instance);
...
uint8_t Greenface_gadget::get_param_type(int16_t indx)
{
    if (indx == -1)
        indx = param_num;
    if (string_params)
    {
        if (string_params[indx].length())
        {
            if (string_params[indx] == STRING_TOKEN)
            {
...
String Greenface_gadget::get_param_as_string(int16_t indx)
{
    ...
    case SPANK_STRING_VAR_TYPE:
        return get_param_as_string_var(indx);
    case SPANK_FIXED_POINT_TYPE:
        return String(get_param_as_float(indx), get_decimal_places(indx));
    default:
        return String(get_param_w_offset(indx));
    }
    // return csv_elem(string_params[indx], ',', get_param(indx));
}
...
int Greenface_gadget::get_decimal_places(int16_t indx)
{
    if (indx == -1)
        indx = param_num;
    if (decimal_places)
    {
        return decimal_places[indx];
    }
    else
    {
        return 0;
...
void Greenface_gadget::init_param(int8_t indx)
{
    if (indx == -1)
        indx = param_num;
    switch (get_param_type(indx))
    {
    case SPANK_STRING_PARAM_TYPE:
        if (string_param_init_vals)
        {
            string_params[indx] = string_param_init_vals[indx];
        }
        break;
    case SPANK_STRING_VAR_TYPE:
        // ui.terminal_debug("Setting default for string var: " + String(indx) + " to: " + String(init_stringvars[indx]));
        // Serial.println(" string vars index: " + String(get_param(indx)));
        if (init_stringvars)
        {
            string_vars[get_param(indx)].put(init_stringvars[indx]);
        }
        else
        {
            string_vars[get_param(indx)].put(" ");
        }
        break;
    }
    // put_param(init_vals[indx], indx); // put param and print screen
    param_put(init_vals[indx], indx);
    // exe_update_fxn(indx);
}
...
void Greenface_gadget::adjust_param(int e, unsigned long delta)
{
    ...
    default:
        ...
        // ui.terminal_debug("param num: " + String(param_num) + " the_param: " + String(the_param) + " " + name);
        put_param(the_param);
        break;
    }
}
...
bool Greenface_gadget::is_available(int16_t indx)
{
    if (indx == -1)
        indx = param_num;
    if (alt_values)
    {
        String alt_value = alt_values[indx];
        return (alt_value != NOT_AVAILABLE);
    }
    else
    {
        return true;
...
void Greenface_gadget::print_param(uint16_t p_num, uint16_t line_num)
{
    ...
    default:
        ...
        if (units)
        {
            my_units = units[p_num];
        }

        // ui.terminal_debug("Param: " + String(param) + " ndigs: " + String(ndigs) + " fmt: " + fmt + " is offset: " + String(is_offset));
        ui.printParam(get_label(p_num), param, ndigs, format, line_num, 1, my_units);
        break;
    }
}

void Greenface_gadget::exe_update_fxn(int8_t _param_num)
{
    if (_param_num != -1)
    {
...
void Greenface_gadget::param_put(uint16_t val, int8_t _param_num)
{
    ...
    {
        _param_num = constrain(_param_num, 0, num_params - 1);
    }
    val = constrain(val, get_min(_param_num), get_max(_param_num));
...
void Greenface_gadget::put_param(uint16_t val, int8_t _param_num)
{
    ...
    printParam();
    // ui.terminal_debug("Put_param Param num: " + String(param_num) + " Val: " + String(val));
    exe_update_fxn();
...
String Greenface_gadget::param_toJSON(int i)
{
    String out = "";
    String label;
    uint8_t param_type;
    bool alt_done, dignum_done;
    String tempval;
    ...
    if (get_min_w_offset(i) < 0 || decimal_places[i] > 0)
    {
        num_digits += -1;
    }

    dignum_done = false;
    int dig_num = digit_num;
    if (offsets)
    {
        if (offsets[i] != 0)
        {
            dig_num = max(0, digit_num - 1);
            dignum_done = true;
        }
    }
    if (decimal_places)
    {
        if (decimal_places[i] > 0 && !dignum_done)
        {
            dig_num = max(0, digit_num - 1);
        }
    }
    ...
    out += ", ";
    out += toJSON(F("dig_num"), String(dig_num)); //
    out += ", ";
    ...
    if (is_slider)
    {
        out += toJSON(F("is_slider"), String(is_slider[i]));
    }
    else
    {
        out += toJSON(F("is_slider"), "0");
    }
    out += ", ";
    if (cmd)
    {
        out += toJSON(F("cmd"), cmd[i]);
    }
    else
    {
        out += toJSON(F("cmd"), "");
...
String Greenface_gadget::selected_param_toJSON()
{
    String out = param_toJSON(param_num);
...
void Greenface_gadget::init()
{
    ...
    for (int i = 0; i < num_params; i++)
    {
        // Serial.println(String(i) + ": " + get_label(i) + String(init_vals[i]));
        param_num = i;
        init_param(i);
    }
    param_num = 0;
...
uint8_t Greenface_gadget::get_num_digits(int this_param_num, bool numeric_only)
{
    ...
    if (decimal_places)
    {
        has_dp = decimal_places[this_param_num] > 0;
    }
    if (offsets)
    {
