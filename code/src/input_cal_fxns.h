...
uint16_t _input_cal_init_vals[] = {0, -(INPUT_CAL_OFFSET_CORRECTION), 1000, 0};
...
EEPROM_ArrInt input_scale_corrections(adc.scale_correction, NUM_INPUTS);
EEPROM_ArrInt input_offset_corrections(adc.offset_correction, NUM_INPUTS);
...
int get_raw_input_offset_correction(int input)
{
    ...
    return adc.offset_correction[input];
}
...
{
    // ui.terminal_debug("Update input_cal_update_params: ");
...
{
    static uint cal_next_time = 0;
    int param_num = input_cal_fxn.param_num;
    String out = F("Apply 0V to set Offset, 4V to set Scale.");
    int input = input_cal_fxn.get_param(INPUT_CAL_INPUT_NUM);
    int val = adc.read(input);
    float reading = adc.adc_to_volts(val);
    // return "Input: " + String(input) + " Reading: " + String(reading) + " Value: " + String(val) + " raw: " + String(raw);
    if (millis() > cal_next_time)
    {
        out += "<h2>" + String(reading) + " VDC</h2> ";
        out += "Raw ADC: " + String(val) + "<br>";
        send_msg_to_USB(out);
        cal_next_time = millis() + 250;
    }
