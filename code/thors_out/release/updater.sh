...
echo "$board_list" | while read -r line; do
    if [[ $line == *"/dev/cu"* ]]; then
        ...
        if [[ $line == *"Arduino Nano RP2040 Connect"* ]]; then
            ...
            ports+=("$port")
            ((index++))
        elif [[ $line == *"Greenface RP2040"* ]]; then
            echo "$index: Thor's Arsenal found on $port"
            ports+=("$port")
            ((index++))
        fi
    fi
...
./bin/arduino-cli upload -p $selected_port -b arduino:mbed_nano:nanorp2040connect -i ./src.ino.bin
